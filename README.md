...
```bash
python -m cop_agent.reference_v3 serve \
  --host 0.0.0.0 --port 8801 --peer https://their-host/mcp \
  --role police --group-id sparring-s82kma9e --policy search \
...
**The cop's actions mix physics and psychology.** Each turn is one of two
physical actions: a move — `N`, `S`, `E`, `W`, `STAY`
(`domain/actions.py`), diagonals illegal, with `legal_moves` in the domain
layer as the single source of legality — or a barrier placement that
*forfeits* the move, spends one of fourteen irreversible barriers, and must
be declared truthfully with its exact location. That forfeit is what makes
the action space genuinely a decision space: pursuit and construction compete
for the same turn, and section 3 is largely about pricing that competition.
The communicative action is the verbal hint: free natural language, permitted
to be a deliberate lie, composed by the bluff modules (`domain/bluff.py` and
its siblings).
...
capture — twenty points against five for letting the clock run out — and the
0/0 row is load-bearing: it is why so much of section 2 below is about not
deadlocking, and why the barrier scorer refuses a placement that leaves the
cop itself with no legal move.

**The observation Ω𝒸ₒₚ is the heart of the model.** Per turn the cop
observes exactly: its own position, the legality of its own candidates, the
opponent's disclosed scent field, and the opponent's hint. The thief's cell
is never in it — not even at the moment of a capture claim, which is why the
claim must be answered rather than checked. The Live GUI's `render()` has no
parameter for the opponent's true position — the observation model enforced
as an API signature, not as a promise.
...
the update weighted by a reliability earned rather than assumed. The
rulebook's worked example (PDF p. 47) is this agent's daily bread: the thief
...
indistinguishable from a reliable one. When a contradicted hint splits the
posterior into two clusters, `domain/foci.py` commits to one rather than
letting the argmax oscillate — with thirty-five turns, the turns spent
walking between two good guesses are the ones that decide the sub-game.

**γ appears structurally, not as a constant.** The cop's future-regard is
the barrier budget: a placement is a permanent cost paid now for containment
later, so the spending curve in `strategy/budget.py` demands more value from
a barrier the more diffuse the belief is, and withholds a three-barrier
reserve for the endgame squeeze. That is patience — the rulebook's own
example of a high γ is a barrier trap built over many turns — expressed as
an admission threshold rather than a discount coefficient. The uncertainty
the formalism makes symmetric is, on this side, mostly a bill: every unit of
belief entropy raises the price of acting on it.
...
cop, a pursuit one barrier short of a twenty-point capture evaporating into
nothing. The decision is three layers answering three different questions.
The Deadline Tracker (`runtime/deadline.py`) asks *did this call take too
long*: every request carries an expiry, and a missed deadline is a failure,
not an invitation to wait longer. The Watchdog (`runtime/watchdog.py`) asks
*is anything still happening at all*, because the failures that kill matches
are rarely one slow request — a hung model or a transport that neither
returns nor errors trips no per-request timeout — and on a stall it persists
state and shuts down cleanly rather than losing the sub-game. The state
machine (`runtime/state_machine.py`) raises on any transition not in its
table, converting a logic bug into a development-time failure instead of a
silent in-match stall. The wiring between them was a dilemma of its own:
client retries feed the watchdog's heartbeat (`runtime/orchestrator.py`),
because a client patiently retrying a dead tunnel is otherwise silent for
longer than the watchdog's patience — a stall reported over a recovery that
