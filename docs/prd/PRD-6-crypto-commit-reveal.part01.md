...
**Stage 6 of 7** · Rulebook Ch. 5 · Repo: **COP**
...
acknowledge/reveal/audit phases, tamper handling, capture-claim integrity, barrier
declaration integrity, Step-0 declaration and signing, commit-hash reporting, log
integrity.
...
### 4.1 Phase 1 — Commit
- FR-6.1 The agent picks its physical action (move **or** barrier placement) and
  the hint it will send (including the `Intent` flag: is the hint true or false),
  and draws a fresh **Nonce**.
...
- FR-6.4 The sealed record is **richer than the four core fields** — it also
  ...
  the sub-game id, and (for this agent) **the barrier placement, if any**.
- FR-6.5 Field semantics:
  - `State` — the board snapshot the action is based on; prevents reusing an old
    commitment in a new context.
  - `Move` — the physical action (movement, barrier placement, and so on); the core
    we are locking against change.
...
### 4.4 Capture-claim and barrier integrity (cop-critical)
- FR-6.14 A **Capture Claim** places the thief under a cryptographic obligation to
  answer truthfully — and binds us equally. The claim must be derivable from
  verified board state: coordinate overlap, a trapping barrier placement
  (PRD-1 FR-1.12), or enclosure (PRD-1 FR-1.13).
- FR-6.15 There must be **no code path that emits a Capture Claim not implied by
  `BoardState`**. A false claim is exposed at the log-audit stage and triggers
  **immediate disqualification with no appeal** (Appendix E rules 21–22).
- FR-6.16 Every barrier placement is sealed into the commitment **and** declared
  openly with its exact location in the same turn. Hidden barriers and false
  locations are serious disqualification offences (Appendix E rules 15–16). The
  audit re-verifies that every declared barrier matches the committed one.

### 4.5 Step-0 and computational fairness
- FR-6.17 Before the first move, each side collects and publishes its machine
  specification: **OS, CPU cores and frequency, RAM, GPU/VRAM, and the name of the
  language model in use**.
- FR-6.18 The declaration also records the **code version**, the **team name**, the
  **sub-game number**, and — mandatory — the **GitHub commit hash** the code is
  running at for this match.
- FR-6.19 Code may be changed, updated and improved between matches, **but every
  ...
  `github_commit`.
- FR-6.20 The whole specification is packed into a JSON string and **signed
  cryptographically with a pre-supplied key** so it cannot be forged retroactively.
- FR-6.21 All LLM **token consumption** is metered and cryptographically locked as
...
- FR-6.22 The lecturer applies a **normalisation formula** granting bonuses to
...
- FR-6.23 Commit-Reveal embodies the spirit of a **zero-knowledge proof**: each
