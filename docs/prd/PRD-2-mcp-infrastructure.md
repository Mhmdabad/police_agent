...
**Stage 2 of 7** · Rulebook Ch. 2 · Repo: **COP**
...
- FR-2.1 Each agent is **simultaneously an MCP server** (exposing `@mcp.tool`
  ...
  strong side and no weak side — the peers are fully equal in their network role.
...
- FR-2.4 Cop code and thief code run in **two completely separate processes**,
  under **separate config directories** (`config/police/` here vs `config/thief/`
  in the thief repo).
...
| `declare_barrier(position, step)` | **Cop-specific:** truthful, explicit barrier declaration |
| `capture_claim(step)` | **Cop-specific:** issue a Capture Claim the thief must answer truthfully |
...
- FR-2.7 Every tool validates its input and **never trusts an unverified move**.
  ...
  crashing.
- FR-2.8 `declare_barrier` is emitted for **every** placement, with the exact
  location, in the same turn it is placed. There is no code path that places a
  barrier without declaring it.

### 3.4 Orchestrator (Appendix E rule 3)
- FR-2.9 A single `Orchestrator` acts as the **only gateway** to all subsystems:
  MCP Connector, Decision Module, Log Manager, Deadline Tracker, Watchdog.
- FR-2.10 Peripheral modules **never reference each other** — only the
  Orchestrator. Replacing the decision engine must touch exactly one module.
- FR-2.11 The Orchestrator coordinates; it contains **no** decision logic and no
  low-level transport code.

### 3.5 State machine (Appendix E rules 4–5)
- FR-2.12 Legal transitions only:
  ...
  ```
- FR-2.13 An illegal transition raises **immediately** rather than leaving the
...
- FR-2.14 **Deadline Tracker** — every outgoing MCP request carries a timestamp
  ...
  **failure**, not a reason to keep waiting: perform a controlled retry
  ...
  the turn cleanly.
- FR-2.15 **Watchdog** — an independent background process monitors the main loop
...
### 3.7 Turn scheduling
- FR-2.16 Strict alternation; acting out of turn is impossible by construction
  (the state machine plus the turn banner lock in PRD-7).
- FR-2.17 A full turn = cop move + thief move. Scent decay (PRD-4) fires at the
  end of a full turn.
...
if __name__ == "__main__":
    mcp.run(transport="http", host="0.0.0.0", port=8801)
```
...
- [ ] Every barrier placement produces a matching `declare_barrier` call — verified
      by a test that no placement path bypasses declaration.
