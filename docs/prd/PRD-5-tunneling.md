...
**Stage 5 of 7** · Rulebook Ch. 2 · Repo: **COP**
...
- FR-5.2 Most machines sit behind a firewall and NAT and are therefore not
  ...
  traversal** — the fundamental P2P communication problem that protocols such as
  STUN exist to solve by discovering a private host's public address. The practical
  result: the opponent, anywhere in the world, can reach our server.
...
- FR-5.5 `opponent_url` in `config/police/game.toml` switches from
...
- FR-5.7 **Tunnel robustness is part of game robustness.** If one tunnel drops,
  ...
  scheduling. The Deadline Tracker (PRD-2 FR-2.14) must convert that into a
  controlled retry and, ultimately, a clean `TECHNICAL_LOSS` — never an infinite
  wait. Remember that a technical loss scores **0 for both sides**, so a dropped
  tunnel destroys a winning board position too.
...
- FR-5.9 Retry policy: `max_retries` = 3 with `retry_backoff_sec` = 5, applied to
  transport failures only — **never** to re-send a different move or a different
  barrier declaration after a commit.
