...
**Stage 4 of 7** · Rulebook Ch. 4 + Ch. 6 · Repo: **COP**
...
classification, lie detection by physics contradiction, belief-driven pursuit and
containment.
...
- FR-4.6 With ρ = 0.10 a single deposit stays above half-peak for roughly six to
  seven turns. Because the thief's trail is a short film rather than a snapshot,
  the cop can read *direction of travel*, not just a last-known cell.
...
- FR-4.10 Maintain a `grid_size × grid_size` matrix `b(s)` = probability the
  hidden **thief** occupies cell `s`. (Fully symmetric: the thief maintains the
  mirror map about us and uses it to plan escape routes.)
- FR-4.11 On every incoming observation, update by **Bayes' rule** combining two
  evidence sources:
  1. the thief's sampled scent field (trustworthy physics), and
  2. the thief's verbal hint (possibly false) — weighted by a **reliability
     coefficient**.
...
- FR-4.14 `argmax_s b(s)` is the pursuit target. The distribution is a cloud, not
  a point — it reflects the uncertainty that survives the update. If a new hint
  contradicts the previous one, probability mass is redistributed: the peak may
  migrate or **split into two foci**, and the policy must then decide which to
  approach first rather than oscillating between them.
- FR-4.15 **Barrier scoring is belief-weighted:** the value of sealing a region is
  the escape-area reduction multiplied by the belief mass in that region.

### 3.4 Lie detection by physical contradiction — the cop's core exploit
- FR-4.16 For each incoming hint, compute the scent field the claim would imply
  and compare it to the measured field. Worked example from the book: the thief
  says "I moved north"; a fresh northern trail would read ≈ (1−ρ)·0.9 ≈ 0.81, but
  the north measures **0.00** while all the scent mass sits in the south-east
  (τ = 0.81 at `(1,4)`, τ = 0.63 at `(1,3)`). The gap between expected and measured
  is absolute → the thief is lying, with high confidence.
- FR-4.17 On detection: lower the hint reliability coefficient, re-weight belief
  toward the true scent source, and re-aim the pursuit vector — **not** toward the
  declared north, but toward the real scent origin. The thief's manipulation
  becomes a double-edged sword: the very attempt to mislead betrays its position.
- FR-4.18 Note precisely *what* is being detected: the scent map cannot lie, so
  there is no such thing as a "false trail". What is exposed is a **false verbal
  hint**, caught because the trustworthy environment contradicted it.
- FR-4.19 **Applied to ourselves:** the same procedure is available to the thief,
  which cross-checks *our* trail against *our* hints. Our hints must therefore be
  consistent with our own emitted field.

### 3.5 Verbal channel
- FR-4.20 Inter-agent communication about position/intent is **free natural
...
- FR-4.21 Each hint is capped at `hint_max_words` (default **15**). The cap applies
  to the template mode *and* is stated to the LLM in its system prompt.
- FR-4.22 Hints may be seasoned with real landmarks from the agreed `map_area`
  (default `"New York"`), e.g. "closing in near Times Square". Empty `map_area` ⇒
  generic landmarks. `map_area` and `hint_max_words` are negotiated and signed like
  any other match condition.
- FR-4.23 Every hint carries an **`Intent` flag** — `truth` or `lie` — chosen
...
- FR-4.24 The LLM composes bluff text and profiles the thief's language (bluff
  classifier / behavioural profiler). It **never** decides the move or the barrier
  (see PRD-3 FR-3.15/3.16). The division of labour — **language to the model,
  space to the algorithm** — is the core design decision of the agent.
- FR-4.25 Four operating modes, selected in `config/police/game.toml`
  (`[trash_talk] provider`):
...
- FR-4.26 `every_n_steps` invokes the model only once every N turns, cutting
...
- FR-4.27 `step_deadline_seconds` (default 30) hard-caps LLM thinking per step;
  on timeout, fall back to `template` rather than stalling the turn.
- FR-4.28 Total tokens consumed are metered and reported in the final JSON.
