...
**Stage 7 of 7** · Rulebook Ch. 9 + Ch. 7 + Appendix A · Repo: **COP**
...
- FR-7.1 Each side runs its **own** GUI (Tkinter or PyQt) showing **only local
  truth**: our own position, the scent we sense, the barriers we have placed, and
  the hints we received.
- FR-7.2 **There is no bird's-eye view.** Displaying the full objective board state
  ...
  illegal advantage** (Appendix E rules 8–9). In particular the cop's window must
  never show the thief's actual cell, only the belief estimate.

### 3.2 Belief heatmap
- FR-7.3 A dynamic grid renders our belief map about the **thief's** position:
  higher probability ⇒ deeper red. The cell marked `T?` is the current
  `argmax b(s)`; `C` marks our own position; dark cells are barriers (zero belief).
- FR-7.4 The map updates in real time from the thief's scent field and the hints
  received. The student reads a focus of suspicion at a glance instead of a matrix
  of numbers.
...
- FR-7.7 Once we have chosen an action, signed it with a Commit and transmitted it,
...
- FR-7.9 Building a **Replay Viewer is a mandatory submission requirement**, not an
  optional extra.
- FR-7.10 It loads the final log file (e.g. `logs/police_match.json` /
...
- FR-7.14 **Screenshots of the viewer showing `Verified OK`, alongside a screenshot
  of the belief heatmap in the Live GUI, are mandatory README content.** The
  requirement is substantive, not formal: the belief map proves the agent really
  performs probabilistic inference under partial observation, and `Verified OK`
  proves the integrity of the match was preserved.
