...
from cop_agent.infra.ceremony import Acknowledgement, Commitment, Reveal, StepCeremony
...
def ceremony(role: str = "police") -> StepCeremony:
    return StepCeremony(step=4, role=role)


def committed() -> StepCeremony:
    step = ceremony()
    step.commit(Commitment(step=4, sender="police", commit=OURS, timestamp=WHEN), NONCE)
    return step


def locked() -> StepCeremony:
    step = committed()
    step.receive(Commitment(step=4, sender="thief", commit=THEIRS, timestamp=WHEN))
    step.acknowledge(WHEN)
    step.receive_ack(Acknowledgement(step=4, sender="thief", acknowledges=OURS, timestamp=WHEN))
    return step
...
def played() -> StepCeremony:
    step = locked()
    step.reveal(
        Reveal(step=4, sender="police", move="N", intent="truth", hint="north", timestamp=WHEN)
    )
    return step
...
class TestTheLockIsDerivedNotTracked:
    ...
    def test_a_fresh_step_is_open_again(self) -> None:
        ...
        assert banner(StepCeremony(step=5, role="police")).accepting_input
