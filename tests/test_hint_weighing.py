...
:func:`~cop_agent.domain.hints.parse` turns a sentence into a claim,
:func:`~cop_agent.domain.credibility.check` scores that claim against the
trail, and :func:`~cop_agent.domain.inference.update` had accepted ``claim``
...
from cop_agent.domain.credibility import MIN_RELIABILITY, START_RELIABILITY
...
class TestTheHintActuallyMovesTheBelief:
    def test_a_supported_claim_raises_the_cells_it_names(self, tmp_path: Path) -> None:
        ...
        from cop_agent.domain.inference import update as absorb
