...
from cop_agent.strategy.containment import (
    SHRINK_THRESHOLD,
    WINDOW,
    ContainmentTracker,
)
from cop_agent.strategy.thief_brain import ThiefBrain, manhattan
...
class TestAvailableToTheScorer:
    ...
    def test_the_trend_is_logged_every_turn(self, caplog: pytest.LogCaptureFixture) -> None:
        brain = ThiefBrain(axes=AXES)
        with caplog.at_level(logging.INFO, logger="cop_agent.strategy.thief_brain"):
            brain.decide(make(step=0))
