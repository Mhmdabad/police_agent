...
OURS = HERE / "src" / "cop_agent"


def sibling() -> Path | None:
    """The other agent's package, if it is checked out beside this repository.

    Both spellings of the directory are tried: the thief repository is named
    ``theif_agent`` upstream, and a clone that corrected the typo locally is
    still the sibling we mean.
    """
    for name in ("theif_agent", "thief_agent"):
        candidate = HERE.parent / name / "src" / "thief_agent"
        if candidate.is_dir():
            return candidate
    return None
