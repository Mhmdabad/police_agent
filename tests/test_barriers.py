...
class TestBeliefWeightedValue:
    """#63: a barrier is worth the probability it removes, not the cells."""
    ...
    def test_without_a_belief_map_the_raw_count_is_used(self) -> None:
        """Correct before belief exists — and a uniform belief would scale
    ...
    def test_sealing_an_empty_region_is_worth_little(self) -> None:
        """The waste #63 names: one of fourteen barriers and a turn of
        pursuit, spent removing somewhere the thief demonstrably is not."""
    ...
    def test_where_the_mass_sits_decides_between_equal_cuts(self) -> None:
        ...
        The same wall on the same board is worth seven times more when the
        region it removes holds the thief than when it does not. Cell count
        cannot see the difference; the weighted value is the whole gap.
        """
        walls = frozenset({(0, 2), (1, 2), (3, 2), (4, 2), (5, 2), (6, 2)})
        state = board((2, 1), (2, 5), barriers=walls)
    ...
    def test_the_sealed_cell_counts_even_when_nothing_is_cut_off(self) -> None:
        """A wall on the thief's own most-likely square removes that square's
        mass, though it was never cut off from anything."""
        state = board((3, 3), (3, 4))
    ...
    def test_a_uniform_belief_does_not_reorder_anything(self) -> None:
        """It scales every candidate by the same share, so the ranking is the
        one the raw counts produce."""
        state = board((3, 3), (5, 5))
    ...
    def test_the_vetoes_are_unaffected_by_belief(self) -> None:
        """Self-preservation is about the board, not about probability."""
        state = board((0, 0), (2, 2), grid_size=3, barriers={(0, 1), (1, 0)})
    ...
    def test_a_win_is_still_taken_regardless_of_belief(self) -> None:
        """#45 runs before any of this, and must keep doing so."""
        state = board((3, 3), (3, 4))
        assert winning_placement(state, AXES) == (3, 4)
