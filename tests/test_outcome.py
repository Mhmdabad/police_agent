...
class TestOnlyTheBoardCanProduceAClaim:
    ...
    def test_standing_on_the_thief_yields_a_claim_naming_the_cell(self) -> None:
        """ "I captured you" is not checkable; "at (3,3)" is."""
    ...
    def test_it_takes_the_board_and_nothing_else(self) -> None:
        """No argument can express *wanting* to claim.

        A policy cannot pass a flag and a brain cannot pass a preference; the
        only way to obtain a claim is to be in a position that already is one.
        A false claim disqualifies the team outright, with no appeal — a
        harsher penalty than losing every match in the series.
        """
...
class TestAClaimIsCheckedBeforeItIsSent:
    ...
    def test_the_right_capture_at_the_wrong_cell_fails(self) -> None:
        """Ours can be right about the capture and wrong about the cell."""
