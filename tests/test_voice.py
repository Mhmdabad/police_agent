...
:meth:`~cop_agent.strategy.base.BrainBase._hint` returned a constant string,
...
from cop_agent.infra.validation import require_hint
from cop_agent.strategy.police_brain import PoliceBrain
from cop_agent.strategy.voice_config import build_voice
...
def hints(brain: PoliceBrain, turns: int = 6) -> list[str]:
...
class TestTheKeyIsNeverCommitted:
    def test_no_api_key_appears_in_the_shipped_config(self) -> None:
        ...
        """
        config = (Path(__file__).resolve().parent.parent / "config/police/game.toml").read_text()
    ...
    def test_the_provider_reads_the_key_from_the_environment(self) -> None:
        ...
        source = (
            Path(__file__).resolve().parent.parent / "src/cop_agent/domain/providers_backends.py"
        ).read_text()
