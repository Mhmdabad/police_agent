...
def place_barrier(state: BoardState, at: Position, axes: AxisConvention) -> BoardState:
    """Seal ``at`` and return a new state, leaving the cop where it stands.

    Raises:
        IllegalActionError: if the placement is not permitted.
    """
...
def apply_action(
    state: BoardState, agent: Agent, action: Action, axes: AxisConvention
) -> BoardState:
    ...
    match action:
        ...
        case PlaceBarrier(at=at):
            ...
            return place_barrier(state, at, axes)
